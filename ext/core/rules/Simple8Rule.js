    ...
    for (const step of steps) {
      ...
      if (actionValue === 5) {
        ...
        if (isUpperActive || inactiveLower < 3) {
          console.error(`❌ Физически невозможно: +8 из ${digitValue} (верх:${isUpperActive}, свободно:${inactiveLower}) в разряде ${position}`);
          return false;
        }
      } else if (actionValue === -8) {
        // -8 требует: верхняя активна, хотя бы 3 активные нижние
        if (!isUpperActive || activeLower < 3) {
          console.error(`❌ Физически невозможно: -8 из ${digitValue} (верх:${isUpperActive}, активно:${activeLower}) в разряде ${position}`);
          return false;
        }
